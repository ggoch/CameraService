...
def generic_cache_update(
    prefix: str, key: str, update_with_page: bool = False, pagenation_key: str = None
):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if update_with_page:
                try:
                    ...
                    old_redis_result: str = rc.zrange(
                        name=page_key,
                        start=value_key,
                        end=value_key,
                        withscores=False,
                    ...
                    print(
                        ...
                        str(
                            merge_dict(
                                merge_dict(
                                    ast.literal_eval(old_redis_result), sql_dict
                                ),
                                {pagenation_key: value_key},
                            )
                        ),
                    )
                    rc.zadd(
        ...
        async def wrapper(*args, **kwargs):
            ...
            if value_key:
                ...
                try:
                    page_key = f"{prefix}_page"
                    rc.zremrangebyscore(
                        name=page_key, min=redis_dict["id"], max=redis_dict["id"]
                    )
                except:
                    pass
